...
public class ArithmeticController {
	...
	public Handler add = (ctx) -> {
		...
		ctx.result(arithmeticService.doOperation(number1Str, number2Str, '+'));
	};
	...
	public Handler subtract = (ctx) -> {
		...
		ctx.result(arithmeticService.doOperation(number3Str, number4Str, '-'));
	};
	...
	public Handler multiply = (ctx) -> {
		...
		ctx.result(arithmeticService.doOperation(number5Str, number6Str, '*'));
	};
	...
	public Handler divide = (ctx) -> {
		...
		ctx.result(arithmeticService.doOperation(number7Str, number8Str, '/'));
	};
